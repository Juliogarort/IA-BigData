    ...
    def procesar_nulos(self) -> None:
        ...
        if self.df_procesado['Kilómetros'].isnull().any():
            for anyo in self.df_procesado['Anyo'].unique():
                mediana = self.df_procesado[self.df_procesado['Anyo'] == anyo]['Kilómetros'].median()
                mask = (self.df_procesado['Anyo'] == anyo) & (self.df_procesado['Kilómetros'].isnull())
        ...
        if self.df_procesado['C_Maletero'].isnull().any():
            ...
            self.df_procesado['C_Maletero'].fillna(media, inplace=True)
        ...
        if self.df_procesado['Emisiones'].isnull().any():
            for combustible in self.df_procesado['Combustible'].unique():
                media = self.df_procesado[self.df_procesado['Combustible'] == combustible]['Emisiones'].mean()
                mask = (self.df_procesado['Combustible'] == combustible) & (self.df_procesado['Emisiones'].isnull())
        ...
        if self.df_procesado['Autonomía'].isnull().any():
            for combustible in self.df_procesado['Combustible'].unique():
                mediana = self.df_procesado[self.df_procesado['Combustible'] == combustible]['Autonomía'].median()
                mask = (self.df_procesado['Combustible'] == combustible) & (self.df_procesado['Autonomía'].isnull())
